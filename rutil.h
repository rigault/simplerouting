...
extern bool   readPolar (const char *fileName, PolMat *mat, int lang);
...
extern char   *newDateWeekDayVerbose (long intDate, double myTime, char *res);
...
extern char   *buildMeteoConsultUrl (int type, int i, char *url);
extern char   *buildWebGribUrl (char *url, int typeWeb, int topLat, int leftLon, int bottomLat, int rightLon, int step);
extern bool   curlGet (const char *url, char *outputFilei, char *errMessage, int lMax); 
...
extern bool   smtpGribRequest (int type, double lat1, double lon1, double lat2, double lon2, char *command, size_t maxLength);
